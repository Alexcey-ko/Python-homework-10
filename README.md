...
По умолчанию pytest создает новый цикл событий под каждый тест и, если использовать асинхронный движок, то это будет ломать тесты, т.к. движок мы создаем в фикстуре со `scope=session`. Чтобы цикл событий использовал тот же самый цикл событий, в котором создан движок, нужно добавить опцию `asyncio_default_fixture_loop_scope = "session"` в `pyproject.toml`. Далее, там, где нужен сессионный цикл событий, нужно использовать дополнение `@pytest.mark.asyncio(loop_scope='session')`.
